...
def index_list():
    ...
    start_dt = '20101130'
    end_dt = "20141130"
    stock_pool = ["000852.SH","000300.SH","000903.SH","000905.SH","000906.SH"]
    #con_list = pro.index_weight(index_code='000906.Sh', start_date="20180505", end_date="20181203")
    #stock_pool = list(set(con_list.con_code.tolist()))
    total = len(stock_pool)
    # 循环获取单个股票的日线行情
    for i in range(len(stock_pool)):
        ...
        for j in range(c_len):
            ...
            state_dt = (datetime.datetime.strptime(resu[1], "%Y%m%d")).strftime('%Y-%m-%d')
            try:
                sql_insert = "INSERT INTO stock_index(state_dt,stock_code,open,close,high,low,vol,amount,pre_close,index_change,pct_change) VALUES ('%s', '%s', '%.2f', '%.2f','%.2f','%.2f','%i','%.2f','%.2f','%.2f','%.2f')" % (
                    state_dt, str(resu[0]), float(resu[3]), float(resu[2]), float(resu[4]), float(resu[5]),
                    float(resu[9]), float(resu[10]), float(resu[6]), float(resu[7]), float(resu[8]))
                Bars.insert(sql_insert)
            except Exception as err:
                continue
